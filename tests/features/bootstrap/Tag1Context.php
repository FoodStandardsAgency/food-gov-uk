...
class Tag1Context extends DrupalContext {
  ...
  public function __construct(array $parameters) {
    ...
    // initialise array for saved nids
    $this->savedNodes = array(); 
...
//    $this->iClearTheCache();
   ...
   */
   ...
   // =========== Helper function
   /**
    * Extract Drupal data from custom div generated by behat_custom module
   **/
  ...
  **/
  ...
  public function iEditTheCurrentNode() {
   ...
   /**
  * @Then /^I save the current node as "([^"]*)"$/
  */
 public function iSaveTheCurrentNodeAs($variableName) {
   try {
      $nid = $this->getDrupalNid();
      if ($nid) {
        $this->savedNodes[$variableName] = $nid;           
      } else {
        throw new \Exception('Can\'t get nid.');
      }
    }
 ...
 /**
  * @Then /^I go to saved node "([^"]*)"$/
  */
 public function iGoToSavedNode($variableName) {
    try { 
      $nid = $this->savedNodes[$variableName];
      if ($nid) { 
        $this->getSession()->visit($this->locatePath('/node/'. $nid)); 
      }
      else  {
        throw new \Exception('Saved node does not exist');
      }
   }
   catch (\Exception $e) {
      echo 'Caught exception: ',  $e->getMessage(), "\n";
